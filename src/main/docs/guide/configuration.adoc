...
NOTE: Micronaut doesn't use the Flyway property `cleanDisabled` because by default Flyway cleans the schema before running migrations. Please use Micronaut's `clean-schema` configuration property to enable it.
