...
- Last full-suite result: build successful, 4 tests executed (2 test classes), 2 skipped (micronaut-core 5.2.3, micronaut-build 8.1.2).
...
- `V3__Migrate_books` extends Flyway's `BaseJavaMigration` like the Java example (version and description are derived
  from the class name); Python has no nested classes, so it is a top-level class of the factory module.
